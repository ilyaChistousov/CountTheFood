...
import ilya.chistousov.countcalories.databinding.FragmentProfileInfoScreenBinding
import ilya.chistousov.countcalories.domain.model.Goal
import ilya.chistousov.countcalories.presentation.register.fragment.RegisterFragmentContainer.Companion.SIXTH_SCREEN
import ilya.chistousov.countcalories.util.CURRENT_GROWTH
import ilya.chistousov.countcalories.util.CURRENT_WEIGHT
import ilya.chistousov.countcalories.util.DESIRED_WEIGHT
    ...
    private fun setNextScreen() {
        with(binding) {
            buttonNextFragment.setOnClickListener {
                ...
                if (currentGoal == Goal.KEEPING_CURRENT_WEIGHT) {
                    selectedDesiredWeight = 0F
                } else {
                    selectedDesiredWeight = inputDesiredWeight.text.toString().toFloat()
                }
